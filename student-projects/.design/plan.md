...
Use a minimal custom cookie-auth flow instead of ASP.NET Core Identity. This keeps the login page small and avoids the extra Identity UI and account-management surface. Seed one default user at startup so v1 only needs a login page, not registration.
...
- Auth is intentionally minimal and production-light: custom cookie auth with one seeded local user, not full ASP.NET Core Identity.
